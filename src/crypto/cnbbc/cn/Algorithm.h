...
public:
    enum Id : int
    {
        ...
        CN_BBC,    // "cn/bbc"       CryptoNight variant 2 (modified, bbc only).
    ...
    inline Algorithm(const char* algo)
      : m_id(parse(algo)) {}
    inline Algorithm(Id id)
      : m_id(id) {}

    inline bool isEqual(const Algorithm& other) const
    {
    ...
    inline bool operator!=(Algorithm::Id id) const
    {
        return m_id != id;
    }
    ...
    inline operator Algorithm::Id() const
    {
        return m_id;
    }
