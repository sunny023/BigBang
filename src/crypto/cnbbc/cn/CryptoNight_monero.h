...
#define VARIANT1_INIT(part)                                                                                                                             \
    ...
    {                                                                                                                                                   \
        tweak1_2_##part = (*reinterpret_cast<const uint64_t*>(input + 35 + part * size) ^ *(reinterpret_cast<const uint64_t*>(ctx[part]->state) + 24)); \
    }
#else
#define VARIANT1_INIT(part)                                                             \
    uint64_t tweak1_2_##part = 0;                                                       \
...
#define BBC_INIT()                                             \
    __m128i _c_aes;                                            \
    __attribute__((aligned(16))) uint8_t sha_data[32] = { 0 }; \
    __attribute__((aligned(16))) uint8_t sha_out[224] = { 0 };

#define BBC_PRE_AES()                                    \
    if (ALGO == Algorithm::CN_BBC)                       \
    {                                                    \
        _mm_store_si128((__m128i*)sha_data, cx);         \
        _mm_store_si128((__m128i*)(sha_data + 16), ax0); \
        keccak(sha_data, 32, sha_out);                   \
    }
...
#define BBC_PPOST_AES()                           \
    if (ALGO == Algorithm::CN_BBC)                \
    {                                             \
        al0 ^= ((uint64_t*)(&_c_aes))[0];         \
        ah0 ^= ((uint64_t*)uint64_t(&_c_aes))[1]; \
        al0 ^= ((uint64_t*)uint64_t(sha_out))[0]; \
        ah0 ^= ((uint64_t*)uint64_t(sha_out))[1]; \
    }

#ifdef _MSC_VER
#define VARIANT2_SET_ROUNDING_MODE() \
